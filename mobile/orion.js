...
let appState = {
  ...
  activeProfileId:'profile_joao', valuesHidden:false, lastPhraseKeys:[], swipeStart:null
};
...
function greetingForHour(hour=new Date().getHours()){
  if(hour>=5&&hour<12)return 'Bom dia, João!';
  if(hour>=12&&hour<18)return 'Boa tarde, João!';
  if(hour>=18||hour<5)return 'Boa noite, João!';
  return 'Olá, João!';
}
...
async function restoreSnapshot(snap){
  for(const store of ['profiles','accounts','transactions','goals','allocations','debts','recurrences','recurrence_months','monthly_snapshots']){
