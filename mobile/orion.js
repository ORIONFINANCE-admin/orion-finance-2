...
async function openAccountsManager(){
  ...
  $('accountsManagerList').innerHTML=(await Promise.all(ordered.map(async a=>{const invoice=a.credit_card_enabled?await openInvoiceAmount(a.id):0;const detail=(brandKey(a)==='inter')?`Garantia ${money(Number(a.cdb_mais_limite||0))}${invoice>0.005?` · Fatura ${money(invoice)}`:''}`:`${typeLabel(a.type)} · saldo atual`;return `<button type="button" class="account-manager-row" data-edit-account="${escapeHTML(a.id)}">${accountLogoHTML(a,'manager-account-logo')}<span><strong>${escapeHTML(a.name)}</strong><small>${escapeHTML(detail)}</small></span><b>${escapeHTML(displayMoney(balances.get(a.id)||0))}</b><i>Editar</i></button>`;}))).join('');
