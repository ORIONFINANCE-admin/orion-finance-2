...
const DEFAULT_ACCOUNTS = [
  ...
  {id:'acc_inter',name:'Inter',institution:'Inter',type:'digital',type_label:'Conta digital',initial_balance:0,color:'#ff7a00',brand:'inter'},
...
function bindEvents(){
  ...
  $('profileSwitchBtn')?.addEventListener('click',()=> $('settingsHubDialog') ? openSettingsHub() : openProfiles());
  $('settingsActiveProfile')?.addEventListener('click',()=>{closeDialog('settingsHubDialog');openProfiles()});
  $('settingsSwitchProfileBtn')?.addEventListener('click',()=>{closeDialog('settingsHubDialog');openProfiles()});
  $('manageAccountsSettingsBtn')?.addEventListener('click',()=>{closeDialog('settingsHubDialog');openAccountsManager()});
  $('manageProfilesBtn')?.addEventListener('click',()=>{closeDialog('settingsHubDialog');openProfiles()});
...
async function updateProfileHeader(){
  ...
  if($('profileAvatar'))$('profileAvatar').textContent=String(p.name||'P').trim().charAt(0).toUpperCase();
  if($('settingsProfileName'))$('settingsProfileName').textContent=p.name;
  if($('settingsProfileAvatar'))$('settingsProfileAvatar').textContent=String(p.name||'P').trim().charAt(0).toUpperCase();
}
...
async function renderAccounts(snapshot=null){
  ...
  if(p?.principal!==false){
    for(const a of accounts){
      const physical=s.balances.get(a.id)||0;
      const allocated=s.managedMap.get(a.id)||0;
      const shown=physical-allocated;
      if(Math.abs(shown)<0.005) continue;
      rows.push(accountRow(a,shown,allocated));
    }
  } else {
    const byAccount=new Map();
    for(const al of s.allocs.filter(x=>x.profile_id===p.id))byAccount.set(al.account_id,(byAccount.get(al.account_id)||0)+Number(al.amount||0));
    for(const [id,val] of byAccount){
      if(Math.abs(val)<0.005) continue;
      const a=accounts.find(x=>x.id===id);
      if(a)rows.push(accountRow(a,val,0,true));
    }
  }
...
async function saveAccount(e){e.preventDefault();const id=$('editingAccountId').value||uid('acc');const old=await getOne('accounts',id);const obj={id,name:$('accountName').value.trim(),institution:$('accountInstitution').value.trim(),type:$('accountType').value,initial_balance:readMoneyInput('accountInitial'),color:$('accountColor').value,brand:old?.brand||brandKey({name:$('accountName').value,institution:$('accountInstitution').value}),active:true,created_at:old?.created_at||nowISO(),updated_at:nowISO()};if(!obj.name)return;await put('accounts',obj);await markDirty();closeDialog('accountDialog');showToast('Conta salva.');await renderAll()}
