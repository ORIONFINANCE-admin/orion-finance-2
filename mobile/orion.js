...
async function ensureKnownDebts(){
  ...
  const seeds=[
    {
      id:'debt_shopee_408_28',name:'Shopee · acordo R$ 102,07',creditor:'Shopee',kind:'formal',
      base_date:'2026-08-29',base_balance:408.28,settlement_amount:102.07,offer_expiry:'',priority:'high',
      monthly_rate:0,annual_rate:0,
      note:'Oferta de quitação informada: R$ 102,07 (aprox. 75% de desconto). Quitação planejada para setembro de 2026. Condição editável e sujeita à validade da oferta.',
      active:true
    },
    {
...
function navigate(view){
  ...
  const map={home:'homeView',movements:'movementsView',planning:'planningView'};
...
async function renderAll(){await updateProfileHeader();await renderHome();await renderMovements();await renderPlanning();}
async function updateProfileHeader(){if($('profileAvatar'))$('profileAvatar').textContent='J';}
