...
const DB_VERSION = 2;
const STORES = ['meta','profiles','accounts','transactions','goals','allocations','debts'];
...
function bindEvents(){
  ...
  $('syncSettingsBtn')?.addEventListener('click',()=>{closeDialog('settingsHubDialog');openSync()});
  ...
  ['movementValue','accountInitial','interCdbGuarantee','goalTarget','goalMonthly','contributionValue','debtBaseBalance','debtPaymentValue','debtSimulationPayment'].forEach(id=>$(id)?.addEventListener('focus',e=>e.target.select()));
...
async function profileSnapshot(){
  ...
  const expense=month.filter(t=>t.type==='saida'&&!t.is_invoice_payment).reduce((sum,t)=>sum+Number(t.amount||0),0);
  ...
  return {profile,isPrincipal:true,total,income,expense,result:income-expense,free,balances,managedMap:new Map(),accounts,ownGoals,allocs};
}
...
async function renderMore(){const url=await metaGet('sync_url','');if($('syncSettingsSubtitle'))$('syncSettingsSubtitle').textContent=url?'Automático ao abrir e ao reconectar':'Somente neste aparelho'}
...
async function payInterInvoice(){
  ...
  await markDirty();showToast('Fatura paga sem duplicar a despesa.');await renderAll();await renderAccountProfile();
}
...
async function saveGoal(e){e.preventDefault();const account=$('goalAccount').value;if(!account)return showToast('Cadastre uma conta primeiro.');const obj={id:uid('goal'),name:$('goalName').value.trim(),description:$('goalDescription').value.trim(),profile_id:'profile_joao',account_id:account,target:readMoneyInput('goalTarget'),monthly:readMoneyInput('goalMonthly'),created_at:nowISO(),updated_at:nowISO()};if(!obj.name)return;await put('goals',obj);await markDirty();closeDialog('goalDialog');showSmartPhrase('objetivo');await renderAll()}
...
function debtCard(debt,current){
  ...
  const rateText=rate>0?`${formatRateBR(Number(debt.monthly_rate||rate*100))}% a.m.`:'Sem juros';
  const html=`<button class="debt-card ${rate>0?'with-interest':'no-interest'}" data-debt-id="${escapeHTML(debt.id)}"><span class="debt-icon">${rate>0?'↗':'—'}</span><span class="debt-copy"><strong>${escapeHTML(debt.name)}</strong><small>${escapeHTML(debt.creditor|| (debt.kind==='informal'?'Dívida informal':'Dívida formal'))} · ${escapeHTML(rateText)}</small><span class="debt-mini-progress"><i style="width:${pct}%"></i></span></span><span class="debt-value"><b>${escapeHTML(displayMoney(current))}</b><small>${pct}% reduzido</small></span><span class="chev">›</span></button>`;
  return {balance:current,html};
}
...
async function snapshot(){const out={version:APP_VERSION,updated_at:await metaGet('local_updated_at',nowISO()),revision:await metaGet('sync_revision',0),data:{}};for(const s of ['profiles','accounts','transactions','goals','allocations','debts'])out.data[s]=await getAll(s);return out}
async function restoreSnapshot(snap,{mark=false}={}){for(const s of ['profiles','accounts','transactions','goals','allocations','debts']){await clearStore(s);for(const x of (snap.data?.[s]||[]))await put(s,x)}if(snap.revision!==undefined)await metaSet('sync_revision',snap.revision);if(mark){await markDirty()}else await metaSet('dirty',false);let joao=await getOne('profiles','profile_joao');if(!joao){joao={id:'profile_joao',name:'João H.',type:'pessoal',subtitle:'Principal',principal:true,created_at:nowISO(),updated_at:nowISO()};await put('profiles',joao)}else if(joao.name!=='João H.'){await put('profiles',{...joao,name:'João H.',principal:true,updated_at:nowISO()})}for(const p of await getAll('profiles')){if(p.id!=='profile_joao')await remove('profiles',p.id)}appState.activeProfileId='profile_joao';await metaSet('active_profile_id','profile_joao');await renderAll()}
