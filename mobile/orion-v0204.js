...
let appState = {
  ...
  activeProfileId:LOCAL_PROFILE_ID, displayName:'', valuesHidden:false, lastPhraseKeys:[]
};
...
async function completeOnboarding(){
  const p=await currentProfile();if(!p)return;await put('profiles',{...p,onboarding_completed:true,intro_home_seen:true,intro_navigation_seen:true,updated_at:nowISO()});closeDialog('onboardingDialog');showToast('Seu Orion está pronto.');
}
...
function bindEvents(){
  ...
  $('homePriorityCard')?.addEventListener('click',()=>navigate('planning'));
  $('editAccountProfileBtn')?.addEventListener('click',()=>{const id=activeAccountProfileId;closeDialog('accountProfileDialog');if(id)openAccount(id)});
...
async function renderAccountsPage(){
  ...
  if($('accountsListCount'))$('accountsListCount').textContent=`${active.length} ${active.length===1?'conta':'contas'}`;
  const rows=[];
  for (const a of active){
    ...
    const secondaryLines=key==='inter'
      ? [`Garantia · ${money(guarantee)} (CDB)`, ...(invoice>0.005?[`Fatura · ${money(invoice)}`]:[])]
      : [typeLabel(a.type)];
    rows.push(`<button class="accounts-page-row ${key?`brand-${key}`:''}" data-account-page-id="${escapeHTML(a.id)}" style="--account-color:${escapeHTML(a.color||'#2f80ed')}">${accountLogoHTML(a,'accounts-page-logo')}<span><strong>${escapeHTML(a.name)}</strong>${secondaryLines.map(line=>`<small>${escapeHTML(line)}</small>`).join('')}</span><b>${escapeHTML(displayMoney(balance))}</b><i>›</i></button>`);
  }
  $('accountsPageList').innerHTML=rows.join('')||'<div class="empty-state list-card"><strong>Nenhuma conta ativa.</strong>Adicione uma conta para começar.</div>';
...
function movementIconKey(t){
  if(t?.is_invoice_payment) return 'invoice';
  if(t?.type==='transferencia') return 'transfer';
  if(t?.type==='rendimento') return 'growth';
  const category=String(t?.category||'').toLowerCase();
  const desc=String(t?.description||'').toLowerCase();
  if(t?.type==='entrada'){
    if(category.includes('salário')||category.includes('adiantamento')||desc.includes('salário')) return 'salary';
    if(category.includes('benefício')) return 'benefit';
    if(category.includes('reembolso')||category.includes('estorno')||category.includes('restituição')) return 'refund';
    return 'income';
  }
  if(category.includes('aliment')) return 'food';
  if(category.includes('moradia')) return 'home';
  if(category.includes('transporte')) return 'transport';
  if(category.includes('saúde')) return 'health';
  if(category.includes('educação')) return 'education';
  if(category.includes('lazer')) return 'fun';
  if(category.includes('assinaturas')) return 'subscription';
  if(category.includes('compras')) return 'shopping';
  if(category.includes('família')) return 'family';
  if(category.includes('taxas')) return 'fees';
  return 'expense';
}
function movementIconSVG(key){
  const icons={
    invoice:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M7 3h10v18l-2.2-1.5L12 21l-2.8-1.5L7 21V3Z"></path><path d="m9 12 2 2 4-4"></path></svg>',
    transfer:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M7 7h10"></path><path d="m13 3 4 4-4 4"></path><path d="M17 17H7"></path><path d="m11 21-4-4 4-4"></path></svg>',
    growth:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M4 16 10 10l4 4 6-7"></path><path d="M14 7h6v6"></path></svg>',
    salary:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="6" width="18" height="12" rx="3"></rect><path d="M7 12h4"></path><path d="M14 10h4"></path><path d="M14 14h3"></path></svg>',
    benefit:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21s-7-4.4-7-10a4 4 0 0 1 7-2.6A4 4 0 0 1 19 11c0 5.6-7 10-7 10Z"></path></svg>',
    refund:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M9 9H5V5"></path><path d="M5 9a7 7 0 1 1-1 3.7"></path></svg>',
    income:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19V5"></path><path d="m6 11 6-6 6 6"></path></svg>',
    food:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M7 3v8"></path><path d="M10 3v8"></path><path d="M7 7h3"></path><path d="M8.5 11v10"></path><path d="M16 3c1.7 2 1.7 6 0 8"></path><path d="M16 11v10"></path></svg>',
    home:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="m4 11 8-6 8 6"></path><path d="M6 10.5V20h12v-9.5"></path><path d="M10 20v-5h4v5"></path></svg>',
    transport:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><circle cx="7.5" cy="16.5" r="2.5"></circle><circle cx="16.5" cy="16.5" r="2.5"></circle><path d="M7.5 16.5 10 8h5l2 8"></path><path d="M10 8H8"></path><path d="M15 8h2.5"></path></svg>',
    health:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21s-7-4.4-7-10a4 4 0 0 1 7-2.6A4 4 0 0 1 19 11c0 5.6-7 10-7 10Z"></path><path d="M12 7v6"></path><path d="M9 10h6"></path></svg>',
    education:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M4 6.5C6 5.5 8.2 5 10.5 5c2 0 3.9.4 5.5 1.2"></path><path d="M4 6.5v11c2-1 4.2-1.5 6.5-1.5 2.1 0 3.9.4 5.5 1.2"></path><path d="M20 6.5c-1.1-.5-2.4-.9-4-1.1V17"></path></svg>',
    fun:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M10 20V6l9-2v14"></path><circle cx="7" cy="18" r="3"></circle><circle cx="19" cy="16" r="3"></circle></svg>',
    subscription:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="5" width="16" height="14" rx="3"></rect><path d="M4 10h16"></path><path d="M8 14h3"></path></svg>',
    shopping:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8h12l-1 11H7L6 8Z"></path><path d="M9 8V6a3 3 0 0 1 6 0v2"></path></svg>',
    family:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="8" r="2.5"></circle><circle cx="16" cy="9" r="2"></circle><path d="M4.5 18a4.5 4.5 0 0 1 9 0"></path><path d="M13.5 18a3.5 3.5 0 0 1 7 0"></path></svg>',
    fees:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M7 3h10v18l-2.5-2-2.5 2-2.5-2L7 21V3Z"></path><path d="M9 8h6"></path><path d="M9 12h6"></path></svg>',
    expense:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14"></path><path d="m18 13-6 6-6-6"></path></svg>'
  };
  return icons[key]||icons.expense;
}
function movementToneClass(t){
  if(t?.is_invoice_payment) return 'tone-neutral';
  if(t?.type==='entrada' || t?.type==='rendimento') return 'tone-positive';
  if(t?.type==='transferencia') return 'tone-transfer';
  return 'tone-negative';
}
function movementIconHTML(t){
  const key=movementIconKey(t);
  return `<span class="txn-icon ${movementToneClass(t)} icon-${key}">${movementIconSVG(key)}</span>`;
}
...
async function renderMovements(){
  ...
  $('movementList').innerHTML=[...grouped.entries()].map(([date,items])=>{const dayTotal=items.reduce((s,t)=>s+(t.type==='entrada'||t.type==='rendimento'?Number(t.amount):(t.type==='saida'&&!t.is_invoice_payment)?-Number(t.amount):0),0);const totalClass=dayTotal<0?'negative':dayTotal>0?'positive':'neutral';return `<section class="day-group"><div class="day-header"><div><b>${date===todayISO()?'Hoje':dateBR(date)}</b><span class="day-dot">•</span><strong class="${totalClass}">${dayTotal>0?'+ ':dayTotal<0?'- ':''}${money(Math.abs(dayTotal))}</strong></div><small>${items.length} movimenta${items.length===1?'ção':'ções'}</small></div>${items.map(t=>statementRow(t)).join('')}</section>`}).join('');
...
function statementRow(t){
  ...
  const time=transactionTime(t);
  return `<button class="statement-row" data-transaction-id="${escapeHTML(t.id)}"><span class="statement-icon-wrap">${movementIconHTML(t)}${time?`<small>${escapeHTML(time)}</small>`:''}</span><span class="transaction-copy"><strong>${escapeHTML(t.description||typeName(t.type))}</strong><small>${escapeHTML(meta)}</small></span><span class="transaction-amount"><b class="${cls}">${sign} ${escapeHTML(displayMoney(t.amount))}</b><small>${escapeHTML(paymentLabel(t.payment,t))}</small></span><i class="row-chevron">›</i></button>`;
}
...
async function renderPlanning(){
  ...
  const mk=monthKey();
  const monthTx=(await getAll('transactions')).filter(t=>monthKey(t.date)===mk&&t.type==='saida'&&!t.is_invoice_payment);
  const essential=monthTx.filter(t=>t.purpose==='Essencial').reduce((sum,t)=>sum+Number(t.amount||0),0);
  await renderDebts();
