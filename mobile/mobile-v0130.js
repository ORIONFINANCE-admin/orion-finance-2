...
async function openProfileCenter(){if($("settingsDialog")?.open)closeDialog($("settingsDialog"));const s=getSettings(),name=s.profileName||"João";$("profileCenterName").textContent=name;$("profileCenterAvatar").textContent=name.slice(0,1).toUpperCase();$("profileCenterPlan").textContent=s.plan==="premium"?"Premium":"Free";$("profileNameInput").value=name;try{const st=await getSyncStatus();$("profileCenterSync").textContent=!st?.connected?"Não conectada":st?.online===false?"Offline":"Conectada";}catch{$("profileCenterSync").textContent="Manual";}openDialog($("profileDialog"));}
...
function updateMovementDialogMode(){
  ...
  if($("movementSubmitBtn")) $("movementSubmitBtn").textContent=isTransfer?"Salvar transferência":"Registrar";
...
async function ensureFrontendBuild(){
  try{
    ...
    if("serviceWorker" in navigator && (location.protocol === "https:" || location.hostname === "localhost")){
      const registration=await navigator.serviceWorker.register("./service-worker.js?v=0130",{scope:"./"});
