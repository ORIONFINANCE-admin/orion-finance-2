...
async function renderHomeInsights(dashboard){
  ...
  if(income===0 && expense>0){
    ...
    const usage=income>0?(expense/income)*100:0;
  ...
  const label=score>=78?"Boa":score>=58?"Atenção":"Ajustar";
  const hint=score>=78?"Seu mês está sob controle.":score>=58?"Seu mês pede pequenos ajustes.":"Seu mês pede correção imediata.";
  ...
  if($("financialHealthPulse")) $("financialHealthPulse").textContent=score>=78?"✦":score>=58?"∿":"!";
