...
import { syncNow, getSyncStatus, downloadAll, fetchInvestment, resetRemoteData } from "../core/sync.js";
  ...
  $("accountsList").innerHTML = dashboard.contas.length
    ? dashboard.contas.map(a => {
        ...
        return `<article class="account-card" style="background:${escapeHTML(color)}">
          ...
          ${a.tipo === "saldo_livre" ? `<button class="account-transfer" data-transfer-from="${escapeHTML(a.conta_id)}">Transferir saldo</button>` : ""}
        </article>`;
      }).join("")
...
$("confirmResetBtn").addEventListener("click", async () => {
  if ($("resetConfirmInput").value.trim().toUpperCase() !== "ZERAR") {
    return showToast('Digite "ZERAR" para confirmar.');
  }
