...
async function render() {
  ...
  $("accountsList").innerHTML = dashboard.contas.filter(a => !["reserva","reserva_secundaria","investimento_garantia"].includes(a.tipo)).length
    ? dashboard.contas.filter(a => !["reserva","reserva_secundaria","investimento_garantia"].includes(a.tipo)).map(a => {
        ...
        return `<article class="account-card" style="background:${escapeHTML(color)}">
          ...
          <div class="account-balance">${money(displayBalance)}</div>
          <div class="account-meta">${escapeHTML(a.nome)} · ${escapeHTML(a.tipo || "")}</div>${reserveBalance>0?`<div class="account-meta">Disponível ${money(a.saldo_calculado)} · Reserva ${money(reserveBalance)}</div>`:""}
