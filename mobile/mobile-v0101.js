...
const BUILD_ID="2026-08-16.0500";
...
function bindInstitutionCards(){
  document.querySelectorAll(".institution-card-clickable[data-institution-id]").forEach(card=>{
    const open=()=>openInstitutionStatement(card.dataset.institutionId,card.dataset.institutionName);
    card.onclick=open;
...
async function render() {
  ...
  $("saidasMes").textContent = money(dashboard.saidasMes);
  {
    ...
    if (resultadoMesNode) resultadoMesNode.textContent = money(dashboard.resultadoMes);
  }
...
document.querySelectorAll("[data-brand-option]").forEach(btn=>btn.addEventListener("click",()=>{const next=saveSettings({brandStyle:btn.dataset.brandOption});applyBrand(next.brandStyle);renderBrandPicker(next);showToast("Tema visual atualizado.");}));
...
async function refreshFrontendBuild(){
  try{
    ...
    if("caches" in window){
      ...
      await Promise.all(keys.filter(k=>k.startsWith("orion-mobile-")).map(k=>caches.delete(k)));
    }
  }catch(e){console.warn("Limpeza do frontend",BUILD_ID,e);}
}
