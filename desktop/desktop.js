...
const BUILD_ID="2026-08-15.2355";
...
function brandMarkSVG(style="radar-visionario"){
 const v=normalizeBrandStyle(style), c='viewBox="0 0 64 64" aria-hidden="true" focusable="false"';
 if(v==="monograma-radar")return `<svg ${c} class="orion-brand-svg"><circle class="brand-ring" cx="31" cy="32" r="18"/><circle class="brand-ring faint" cx="31" cy="32" r="10"/><path class="brand-orbit" d="M8 42c11 8 31 7 44-6 5-5 7-10 6-14"/><circle class="brand-core" cx="31" cy="32" r="4"/><path class="brand-axis" d="M31 9v46"/><path class="brand-star" d="m49 10 2.2 5.1L56 17l-4.8 1.9L49 24l-2.1-5.1L42 17l4.9-1.9z"/></svg>`;
 if(v==="visionario")return `<svg ${c} class="orion-brand-svg"><path class="brand-arc" d="M42 13C22 7 10 21 13 39c2 12 13 19 24 14 7-3 11-10 10-19"/><path class="brand-j" d="M39 23v17c0 8-4 12-10 12-5 0-9-3-10-8"/><path class="brand-orbit" d="M8 42c12 7 29 6 41-5 6-5 9-11 9-17"/><path class="brand-star" d="m49 9 2.2 5.1L56 16l-4.8 1.9L49 23l-2.1-5.1L42 16l4.9-1.9z"/></svg>`;
 if(v==="minimal")return `<svg ${c} class="orion-brand-svg"><path class="brand-orbit" d="M7 41c13 9 34 6 46-8 5-6 6-12 5-17"/><circle class="brand-core" cx="19" cy="43" r="4"/><path class="brand-star" d="m49 9 2.2 5.1L56 16l-4.8 1.9L49 23l-2.1-5.1L42 16l4.9-1.9z"/></svg>`;
 return `<svg ${c} class="orion-brand-svg"><circle class="brand-ring" cx="31" cy="32" r="19"/><circle class="brand-ring faint" cx="31" cy="32" r="11"/><path class="brand-axis" d="M31 8v48"/><path class="brand-j" d="M38 23v16c0 8-4 12-10 12-5 0-8-3-9-7"/><circle class="brand-core" cx="31" cy="32" r="4"/><path class="brand-orbit" d="M7 42c12 8 32 7 45-6 5-5 7-10 6-15"/><path class="brand-star" d="m49 9 2.2 5.1L56 16l-4.8 1.9L49 23l-2.1-5.1L42 16l4.9-1.9z"/></svg>`;
}
...
function closeInstitutionStatement(){const overlay=$("institutionStatementOverlay");if(!overlay)return;overlay.classList.add("hidden");overlay.setAttribute("aria-hidden","true");__statementInstitutionId="";}
async function openInstitutionStatement(institutionId,institutionName){
  ...
  const filtered=transactions.filter(t=>accountIds.has(String(t.conta_id||""))).sort((a,b)=>`${b.data||""}|${b.criado_em||""}`.localeCompare(`${a.data||""}|${a.criado_em||""}`));
  ...
  $("institutionStatementMovements").innerHTML=filtered.length?renderMovementList(filtered):`<div class="empty">Nenhuma movimentação vinculada a esta instituição.</div>`;
  const overlay=$("institutionStatementOverlay");overlay.classList.remove("hidden");overlay.setAttribute("aria-hidden","false");
}
function bindInstitutionCards(){document.querySelectorAll(".institution-card-clickable[data-institution-id]").forEach(card=>{const open=()=>openInstitutionStatement(card.dataset.institutionId,card.dataset.institutionName);card.onclick=open;card.onkeydown=e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();open();}};});}
function bindInstitutionStatementDismiss(){$("institutionStatementClose")?.addEventListener("click",closeInstitutionStatement);$("institutionStatementOverlay")?.addEventListener("click",e=>{if(e.target===e.currentTarget)closeInstitutionStatement();});document.addEventListener("keydown",e=>{if(e.key==="Escape"&&__statementInstitutionId)closeInstitutionStatement();});}
...
function openSettings(){const settings=getSettings();$("themeSelect").value=settings.theme;$("animationsToggle").checked=settings.animations;$("hideValuesToggle").checked=settings.hideValues;renderBrandPicker(settings);$("settingsDialog").showModal();}
...
document.querySelectorAll("[data-brand-option]").forEach(btn=>btn.addEventListener("click",()=>{const next=saveSettings({brandStyle:btn.dataset.brandOption});applyBrand(next.brandStyle);renderBrandPicker(next);toast("Tema visual atualizado.");}));
