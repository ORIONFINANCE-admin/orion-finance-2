...
const BUILD_ID="2026-08-16.0415";
...
function bindInstitutionStatementDismiss(){
  $("institutionStatementClose")?.addEventListener("click",e=>closeInstitutionStatement(e));
  ...
  $("institutionStatementOverlay")?.addEventListener("click",e=>{ if(e.target===e.currentTarget) closeInstitutionStatement(e); });
...
$("financialBaseBtn")?.addEventListener("click",async()=>{
  try{
    const pref=$("preferencesDialog");
    if(pref?.open){ if(typeof closeDialog==="function") closeDialog(pref); else pref.close(); }
    await openInvestmentSetup({force:true});
