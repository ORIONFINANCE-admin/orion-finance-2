...
async function reward(context) {
  const text=smartPhrase(context,{force:["aporte","rendimento","divida","reserva"].includes(context),name:getSettings().profileName||""});
  if(!text){toast("✓ Registrado");return;}
  $("rewardText").textContent=text;$("reward").classList.remove("hidden");setTimeout(()=>$("reward").classList.add("hidden"),2400);
}
...
async function render() {
  ...
  try { await renderHomeInsights(dashboard); if($("desktopCompactHealth"))$("desktopCompactHealth").textContent=`● ${$("financialHealthPercent")?.textContent||"Saúde"}`; } catch (e) { console.error("Falha ao renderizar saúde", e); }
...
async function checkSync() {
  if(!getApiToken()||!navigator.onLine)return;
  try{await syncNow();await render();}catch(e){console.warn("Sync automático",e);}
}
