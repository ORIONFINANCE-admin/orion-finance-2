...
export function renderInstitutionCard(item, escapeHTML) {
  ...
  return `<article class="account-card institution-card institution-card-clickable" data-institution-id="${escapeHTML(String(item.instituicao_id||""))}" data-institution-name="${escapeHTML(safeName)}" tabindex="0" role="button" aria-label="Ver extrato de ${escapeHTML(safeName)}" style="background:${escapeHTML(item.color)}">
    <div class="account-title">
      <span class="bank-mark">${escapeHTML(safeName.slice(0,1))}</span>
      ${escapeHTML(safeName)}
    </div>
    <div class="account-balance">${money(item.total)}</div>
    ${breakdown}
    ${cards}
    <div class="institution-card-affordance">Ver extrato <span>›</span></div>
  </article>`;
}
