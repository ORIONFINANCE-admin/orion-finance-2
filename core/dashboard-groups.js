...
export function buildInstitutionCards({ accounts = [], institutions = [], cards = [] } = {}) {
  ...
  for (const account of accounts) {
    ...
    const legacyVirtualName=String(account.nome||"").toLowerCase();
    const isLegacyMercadoPocket=/reserva|maria estela|15 anos/.test(legacyVirtualName) && /reserva|secundaria|cofrinho/.test(String(account.tipo||"").toLowerCase()+" "+legacyVirtualName);
    if (!nearlyZero(balance) && !isLegacyMercadoPocket) {
      item.lines.push({
        id: account.conta_id,
