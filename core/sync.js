...
export async function getSpreadsheetExportURL() {
  const token = requireToken();
  const url = `${ORION_CONFIG.apiUrl}?acao=exportar_xlsx&token=${encodeURIComponent(token)}`;
  const data = await fetchJSON(url);
  if (!data.ok || !data.url) throw new Error(data.erro || "Não foi possível preparar a exportação.");
  return data;
}
