...
export async function calculateDashboard() {
  ...
  const saldoTotal = contas.filter(a => a.tipo !== "investimento_garantia").reduce((sum, a) => sum + Number(a.saldo_calculado || 0), 0);

  const patrimonioTotal = contas.reduce((sum, a) => sum + Number(a.saldo_calculado || 0), 0);
  const cards = await getAll("cartoes_credito");
  const compromissosCartao = cards.reduce((sum,c) => sum + Math.max(0, Number(c.limite_total||0)-Number(c.limite_disponivel||0)), 0);
