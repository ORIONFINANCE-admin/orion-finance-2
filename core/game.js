...
export async function getGameState({transactions=[],debts=[],reserve=null}={}){
 const events=await getAll("eventos_xp"); const xp=events.reduce((s,e)=>s+Number(e.xp||0),0); const current=[...LEVELS].reverse().find(l=>xp>=l.xp)||LEVELS[0]; const next=LEVELS.find(l=>l.level===current.level+1)||null;
