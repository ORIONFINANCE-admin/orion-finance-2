...
export async function updateTransaction(transacaoId, input = {}) {
  const id = String(transacaoId || "");
  if (!id) throw new Error("Movimentação inválida.");
  const current = await getOne("transacoes", id);
  if (!current) throw new Error("Movimentação não encontrada.");

  const tipo = String(input.tipo ?? current.tipo ?? "saida");
  if (!["entrada", "saida"].includes(tipo)) throw new Error("Tipo de movimentação inválido.");
  const value = Number(input.valor ?? current.valor ?? 0);
  if (!(value > 0)) throw new Error("Informe um valor maior que zero.");

  const updated = {
    ...current,
    data: String(input.data ?? current.data ?? todayISO()),
    descricao: String(input.descricao ?? current.descricao ?? "").trim(),
    valor: value,
    tipo,
    categoria_id: String(input.categoria_id ?? current.categoria_id ?? ""),
    subcategoria_id: String(input.subcategoria_id ?? current.subcategoria_id ?? ""),
    conta_id: String(input.conta_id ?? current.conta_id ?? ""),
    forma_pagamento: String(input.forma_pagamento ?? current.forma_pagamento ?? "pix"),
    cartao_id: String(input.cartao_id ?? current.cartao_id ?? ""),
    evento_id: String(input.evento_id ?? current.evento_id ?? ""),
    observacao: String(input.observacao ?? current.observacao ?? "").trim(),
    atualizado_em: nowISO()
  };
  if (!updated.descricao) throw new Error("Informe uma descrição.");
  if (!updated.categoria_id) throw new Error("Selecione a categoria.");
  if (!updated.conta_id) throw new Error("Selecione a conta.");

  await put("transacoes", updated);
  await queueChange("TRANSACOES", updated);
  return updated;
}
